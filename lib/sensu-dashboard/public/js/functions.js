  ...
  $.getJSON('/events.json', function(data) {
    ...
    for (var i in selected_filters) {
      ...
      if (current_filter['type'] == 'client') {
        grouped_filters['clients'] || (grouped_filters['clients'] = []);
        for (var j in current_filter['value']) {
          grouped_filters['clients'].push(current_filter['value'][j]);
        }
      }
      
      // store environment filters in the grouped filters array
      if (current_filter['type'] == 'environment') {
        grouped_filters['environment'] || (grouped_filters['environment'] = []);
        for (var j in current_filter['value']) {
          grouped_filters['environment'].push(current_filter['value'][j]);
        }
      }
    ...
    for (var nodekey in data) {
      ...
      // skip node if environment filters exist and the environment is not in the filter
      if ((Object.size(grouped_filters['environment']) > 0) && ($.inArray(nodekey, grouped_filters['environment']) == -1)) {
        continue;
      }
